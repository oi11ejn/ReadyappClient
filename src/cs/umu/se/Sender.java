...
public class Sender {
    ...
    public static void sendAnswer(final boolean yesOrNo, final String friendId, final String userId) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    ...
                    if (send) {
                        if(yesOrNo) {
                            ClientResource client = new ClientResource("http://" + ips.get(friendId) + "/accept/" + userId);
                            UserId self = new UserId();
                            self.setUserId(userId);
